...
Also, you can enqueue scripts and styles just in the theme, for example when you will display a view for a widget. In this case, you can use [`withStyles()`](/docs/CoreClasses/view#withstyles) and [`withScripts()`](/docs/CoreClasses/view#withscripts) fluent methods. All these methods accept three parameters: name, dependencies, and version:
...
return WPKirk()->view( 'dashboard.index' )
               ->with( [ 'kirk' => 'Captain' ] )
               ->withStyles( 'wp-kirk-common', [ 'wp-kirk-colors' ], '1.0' );
```
...
return WPKirk()->view( 'dashboard.index' )
               ->with( [ 'kirk' => 'Captain' ] )
               ->withScripts( 'wp-kirk-common', [ 'wp-kirk-colors' ], '1.0' );
```
