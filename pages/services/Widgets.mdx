...
As you can see, this new `Widget` class supports the resource view like a view. The `viewForm` method should return the view used in the backend admin area. The `viewWidget` should return the view used in the frontend. In other words, this is the output of widget. In these methods, you may choose which view to return. Inside the view, you'll have available `$plugin` instance, `$instance` WordPress variable and `Html` facade. For example, in `widgets.form` you can:
