...
From release v0.13+ WPBones provides a simple way to handle the WordPress Rest API.
...
First of all, we have added a new `rest-api.php` configuration file to the `config` folder.
...
Of course, you may use a `RestController` to handle the request. WPBones provides a simple way to create a new controller. You may create your own controller extending the `RestController` class.
...
Remember that you have to enebale this feature from `rest-api.php` configuration file.
