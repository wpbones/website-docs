...
Since WP Bones 2.0.12 that connection talks to MySQL the way WordPress does. It uses the charset and collation WordPress settled on (`$wpdb->charset` and `$wpdb->collate`, `utf8mb4` on any recent install, even when `DB_CHARSET` still says `utf8`), so emoji and other 4-byte characters are written and read back intact. It also reads `DB_HOST` like WordPress: a port (`localhost:3307`) and a socket (`localhost:/tmp/mysql.sock`) both work, and an IPv6 host is put in brackets when PHP uses `mysqlnd`, as WordPress does.
