...
This document explains how to localize ReactJS applications in WP Bones. It covers building the application using `yarn build`, generating the `.pot` file with `yarn make-pot`, and editing the `.pot` file for translations. It lists tools for translating PO files online and describes generating the `.json` file with `yarn make-json`. Finally, it explains how to load translations in the application using the `withAdminAppsScript` method.
